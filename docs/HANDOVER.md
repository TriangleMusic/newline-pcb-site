...
>
> מסמך משלים: [`STORAGE-MAP.md`](STORAGE-MAP.md) — איפה כל עותק של הקוד שמור, אילו חשבונות צריך, ואיך משחזרים מגיבוי.
...
│   ├── STORAGE-MAP.md         ← מפת אחסון ופרטי גישה
│   ├── screenshots/           ← ארכיון צילומי מסך של האתר
│   └── dev-logs/              ← לוגים מבדיקות (לא נדרש לתחזוקה)
├── backups/                    ← קובצי ZIP של גיבויים לפי תאריך
...
*נכתב ב-22/05/2026. עודכן ב-21/08/2026 (סידור מבנה התיקיות). כשעושים שינוי משמעותי במבנה — עדכנו את המסמך הזה.*
